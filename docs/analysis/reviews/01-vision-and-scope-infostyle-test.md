# Vision & Scope — тест `sa-infostyle`

> Тестовая копия для проверки навыка `sa-infostyle`. Она не является новой версией `ART-01` и не изменяет утверждённый исходник [`01-vision-and-scope.md`](../01-vision-and-scope.md).
...
Команда не располагает единым способом по изображению и доступным данным определить, рассматривался ли товар ранее под другой карточкой, у другого продавца, на другой площадке или в другом варианте.

Из-за этого накопленная история решений не всегда возвращается при новом рассмотрении. Сотрудники вынуждены полагаться на личную память (`VS-F-002`–`VS-F-004`).
...
Целевой продукт — внутренний инструмент поддержки решения для сотрудников, которые предлагают новые товары, и для отдельных сотрудников, принимающих решения по ним.

По фотографии и доступным данным продукт помогает найти ранее рассмотренные позиции и показывает историю работы с ними. Итог поиска по запросу выражается как `NEW`, `SAME_PRODUCT` или `UNCERTAIN`.

Для конкретного отличающегося кандидата продукт отдельно указывает `VARIANT` или `UNRELATED`. Для подтверждённого `SAME_PRODUCT` отношение имеет значение `N/A`.
...
Возврат существующей истории входит в границу продукта.

Автоматический write-back нового решения не допускается. ЛПР фиксирует окончательное решение и краткое обоснование в согласованном источнике истины.

Сам источник и его схема пока не определены (`DEC-040`, `GAP-006`).
...
Текстовые метаданные, включая название, опциональны и помогают повторной проверке. Они не заменяют обязательную фотографию.

При неполной или противоречивой истории система показывает это ограничение. ЛПР разрешает конфликт и фиксирует краткое обоснование без автоматического выбора.

Точный источник истины, его схема и детальные правила исключений остаются открытым вопросом (`DEC-039`, `DEC-040`, `GAP-006`).
...
Подсчёт полей K/M выполнен для 1 536 записей данных `SRC-003` — логических записей 6–1541. Для подсчёта:

- после удаления пробелов строка учитывалась, если K или M были непустыми;
- контрольная сумма источника: `ac8832e54e915523b0736ffd36f0eb63f6deac5658c5e0c81523abcf07e13623`;
