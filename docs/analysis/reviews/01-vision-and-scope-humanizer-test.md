# Vision & Scope — тест `humanizer-ru`

> Тестовая копия для редакторской проверки навыком `humanizer-ru`. Это не новая версия `ART-01`; утверждённый исходник [`01-vision-and-scope.md`](../01-vision-and-scope.md) не изменён.
...
Целевой продукт — внутренний инструмент поддержки решения для сотрудников, которые предлагают новые товары, и для сотрудников, принимающих решения по ним.

По фотографии и доступным данным продукт находит ранее рассмотренные позиции и показывает историю работы с ними. Итог поиска по запросу: `NEW`, `SAME_PRODUCT` или `UNCERTAIN`.

Для конкретного отличающегося кандидата продукт отдельно указывает `VARIANT` или `UNRELATED`. Для подтверждённого `SAME_PRODUCT` отношение равно `N/A`.
...
Возврат существующей истории входит в границу. Автоматический write-back нового решения не допускается; ЛПР фиксирует окончательное решение и краткое обоснование в согласованном источнике истины. Сам источник и его схема пока не определены (`DEC-040`, `GAP-006`).
...
Текстовые метаданные, включая название, опциональны и помогают повторной проверке; они не заменяют обязательную фотографию. При неполной или противоречивой истории система показывает это ограничение, а ЛПР разрешает конфликт и фиксирует краткое обоснование без автоматического выбора. Точный источник истины, его схема и детальные правила исключений остаются открытым вопросом (`DEC-039`, `DEC-040`, `GAP-006`).
...
До оценки бизнес-эффекта пилота нужно завести отдельную задачу `MEAS-001` (`DEC-024`). Ниже — её предлагаемый минимальный состав. Конкретный состав утверждает владелец задачи:
