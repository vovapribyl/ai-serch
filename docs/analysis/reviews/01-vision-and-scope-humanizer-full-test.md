# Vision & Scope — глубокая редакторская версия

> Это тестовая копия с глубокой редакторской переработкой. Она не является новой версией `ART-01` и не меняет утверждённый [исходник](../01-vision-and-scope.md).
...
| Автор | Codex |
| Утверждённый вход | `ART-00` версии 1.2, `APPROVED_BY_OWNER` |
...
Vision фиксирует проблему, видение продукта, его пользователей, ценность и границы полной системы. Первый прототип здесь рассматривается как проверка гипотез, а не как уменьшенная версия готового продукта.

Документ отвечает на вопросы «зачем нужен продукт» и «что в него входит». Выбор визуальной модели, базы данных, API, платформы запуска и другой технической архитектуры остаётся за его пределами (`DEC-016`, `DEC-009`).
...
Статус `VERIFIED` у `STAKEHOLDER_STATEMENT` подтверждает точность передачи интервью. Он не доказывает частоту или распространённость проблемы независимо от интервью.

### 2.2. Формулировка проблемы

Команда не может по изображению и доступным данным надёжно понять, встречался ли этот товар раньше: под другой карточкой, у другого продавца, на другой площадке или в другом варианте. Поэтому история решений не всегда возвращается в момент новой проверки, а сотрудники опираются на память (`VS-F-002`–`VS-F-004`).

Проблема не сводится к ещё одной отметке `ДУБЛИКАТ AS-IS`. Нужна связь между новой находкой, тем же товаром или его вариантом и историей прошлых рассмотрений (`DEC-001`, `DEC-031`, `DEC-032`).

## 3. Видение продукта

### 3.1. Vision statement

Целевой продукт — внутренний инструмент поддержки решения. Им пользуются сотрудники, предлагающие новые товары, и сотрудники, принимающие решения по ним.

По фотографии и доступным данным продукт ищет ранее рассмотренные позиции и показывает историю работы с ними. Для запроса он возвращает `NEW`, `SAME_PRODUCT` или `UNCERTAIN`. Для конкретного отличающегося кандидата отдельно указывается `VARIANT` или `UNRELATED`; у подтверждённого `SAME_PRODUCT` это поле равно `N/A`.

Текущая проверка сравнивает внешний идентификатор. Новый продукт должен находить связь и при разных продавцах, ссылках, артикулах, площадках, фотографиях и вариантах товара. Окончательное бизнес-решение всё равно принимает сотрудник (`DEC-001`, `DEC-025`, `DEC-026`, `DEC-031`).
...
`ANALOG` не относится ни к одной оси результата и не входит в границу продукта (`DEC-030`, `DEC-031`).
...
| `BG-003` | Снизить зависимость результата от памяти конкретного сотрудника | История доступна любому сотруднику компании через сценарий `CHECK_ONLY` | Текущая зависимость подтверждена интервью, целевой показатель не определён | `VS-F-003`, `DEC-025`, `DEC-026`, `DEC-038` |
...
| — | Сотрудник компании | Выполнить `CHECK_ONLY`: сверить товар с корпусом и изучить историю при наличии совпадения | Это граница доступа, а не новая самостоятельная функциональная роль: сценарий доступен всем сотрудникам, включая инициатора и ЛПР; механизм авторизации остаётся `UNKNOWN` (`DEC-038`, `GAP-002`) |
...
Полный список стейкхолдеров, официальные названия ролей и полномочия появятся в `ART-02` (`GAP-002`).
...
Диаграмма показывает бизнес-границу продукта. Это не схема технической архитектуры.

Продукт возвращает существующую историю. Автоматически записывать новое решение обратно нельзя. ЛПР фиксирует итог и короткое обоснование в согласованном источнике истины; сам источник и его схема пока не определены (`DEC-040`, `GAP-006`).

### 6.4. Сценарии использования

| Сценарий | Кто использует | Ход и граница результата |
|---|---|---|
| `CHECK_ONLY` | Любой сотрудник компании, включая инициатора и ЛПР | Пользователь передаёт обязательную фотографию товара; система сверяет её с корпусом и при наличии кандидата показывает историю решений. Этот сценарий не создаёт запись для рассмотрения и не изменяет корпус. |
| `ADD_FOR_REVIEW` | Инициатор или ЛПР | Пользователь передаёт обязательную фотографию. При результате `NEW` товар сохраняется для дальнейшего анализа и решения ЛПР; при `SAME_PRODUCT` новая запись не создаётся. При `UNCERTAIN` пользователь добавляет фотографии и/или заполняет доступные текстовые метаданные, затем повторяет поиск. Если результат остаётся неразрешённым, запрос и товар не сохраняются в корпусе. |

Название и другие текстовые метаданные необязательны, но могут помочь повторной проверке. Они не заменяют обязательную фотографию.

Если история неполная или противоречивая, система показывает это ограничение. ЛПР разрешает конфликт и фиксирует короткое обоснование; автоматического выбора нет. Точный источник истины, его схема и правила исключений остаются открытыми (`DEC-039`, `DEC-040`, `GAP-006`).
...
Результаты ограниченного прототипа нельзя переносить на весь корпус, другие категории или промышленную нагрузку без отдельной проверки.

Подсчёт K/M сделан по 1 536 записям `SRC-003` — логическим строкам 6–1541. Строка попадала в подсчёт после удаления пробелов, если K или M были непустыми. Контрольная сумма источника: `ac8832e54e915523b0736ffd36f0eb63f6deac5658c5e0c81523abcf07e13623`. URL не открывались, а значения `#REF!` не восстанавливались.
...
До оценки бизнес-эффекта пилота нужна отдельная задача `MEAS-001` (`DEC-024`). Ниже — её предлагаемый минимальный состав. Конкретный состав утверждает владелец задачи:
...
Владелец задачи, период измерения, целевые значения и инструмент сбора пока `UNKNOWN`. Их нельзя подменять предположениями.
...
Пороги утверждены в `DEC-008`, но не станут готовыми критериями приёмки до закрытия `GAP-011`.
...
| `VQ-008` | `DECISION_REQUIRED` | В каком источнике истины ЛПР фиксирует окончательное решение и краткое обоснование, и какова минимальная схема этой записи? | TO-BE, `ART-05`, `ART-07`; `GAP-006` |
...
| История и человеческое решение | `DEC-001`, `DEC-038`–`DEC-040` → `VP-001`, `VP-002`, `SCOPE-005`, `SCOPE-006`, §6.4; источник истины вынесен в `VQ-008` | Нормативные сценарии владельца; автоматический write-back запрещён, механизм фиксации решения ещё не определён |
...
| Авторская проверка поправки v0.7 | `PASSED` | Проверены точность сценариев `DEC-038`–`DEC-040`, границы записи в корпус, открытый `GAP-006`, ссылки, структура Markdown-таблиц и отсутствие новых локальных ID |
| Независимая проверка поправки v0.7 | `PASSED` | [`reviews/01-scenario-decisions-amendment-v0.7-review.md`](01-scenario-decisions-amendment-v0.7-review.md): `PASS`, 0 `Critical`, 0 `Major`, 0 `Minor` |
| Утверждение владельцем поправки v0.7 | `APPROVED` | 2026-08-18: владелец ответил `Подтвержден` (`SRC-031`) |
